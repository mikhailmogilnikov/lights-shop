<script lang="ts">
  ...
  const { products, product }: { products: Product[]; product: Product } = $props();
...
<div transition:blur class="flex items-center gap-4">
  ...
  >
    <button class="text-sm font-bold pb-0.5 px-2 touch-none" onclick={dec}>-</button>
    <p class="text-sm"><NumberFlow value={quantity} /></p>
    <button
      class="text-sm font-bold pb-0.5 px-2 disabled:opacity-50 disabled:cursor-not-allowed touch-none"
